...
- **Post-prediction smoothing** – smooth jagged surfaces from non-smooth
  models (e.g., random forests) via `mgcv::gam()` surrogates
...
{effectsurf} works with any fitted model that supports `predict()`. Here
is a minimal example using a GAM fitted with {mgcv}:
...
post-prediction surface smoothing via `mgcv::gam()` – useful for random
forests, boosted trees, and other non-smooth models.
